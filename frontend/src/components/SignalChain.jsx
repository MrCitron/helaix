        ...
        <div className="p-6">
            <div className="flex items-start justify-between mb-4">
                <div>
                    ...
                    <p className="text-[10px] text-[#586e75] font-mono mt-0.5 tracking-wider">{blockKey.toUpperCase()}</p>
                </div>
                <div className="flex gap-2">
                    <span className="text-[9px] px-2 py-0.5 rounded bg-[#1d2628] text-primary border border-primary/20 font-bold uppercase">{block['@stereo'] ? 'Stereo' : 'Mono'}</span>
                    <button
                        onClick={onClose}
                        className="text-[#586e75] hover:text-white transition-colors"
                    >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {params.length > 0 ? (
                    params.map(([pKey, pVal]) => (
                        <div key={pKey} className="bg-[#111718] border border-[#283639] rounded-lg p-2.5 flex flex-col gap-1 hover:border-[#3b4f54] transition-colors">
                            <span className="text-[10px] text-[#586e75] font-medium uppercase tracking-tight">{pKey}</span>
                            <div className="flex items-center justify-between">
                                <span className="text-white font-mono text-xs">
                                    {typeof pVal === 'number' ? pVal.toFixed(2) : String(pVal)}
                                </span>
                                {typeof pVal === 'number' && (
                                    <div className="w-12 h-1 bg-[#283639] rounded-full overflow-hidden">
                                        <div
                    ...
                    <div className="col-span-full py-4 text-center text-xs text-[#586e75] italic">
        ...
        <div className="w-full transition-all duration-300">
            ...
            <div className="w-full py-6 px-4 overflow-x-auto lg:overflow-visible scrollbar-hide">
                <div className="relative flex flex-col lg:flex-row items-center justify-center min-w-full lg:min-w-max gap-12 lg:gap-8 px-8 py-8 lg:py-0 h-auto lg:h-24">
                    ...
                    <div className="hidden lg:block absolute top-1/2 left-0 w-full h-0.5 bg-[#283639] -translate-y-1/2 z-0"></div>
                    {/* Vertical Signal Line (Mobile) */}
                    <div className="block lg:hidden absolute left-1/2 top-0 w-0.5 h-full bg-[#283639] -translate-x-1/2 z-0"></div>
                        ...
                        return (
                            <React.Fragment key={key}>
                                <div className="relative z-10 flex flex-col items-center">
                                    {/* Connector Dot */}
                                    <div className="absolute lg:top-1/2 top-0 lg:-left-4 left-1/2 w-2 h-2 rounded-full bg-[#3b4f54] -translate-y-4 lg:-translate-y-1/2 -translate-x-1/2 lg:translate-x-0"></div>
                                    ...
                                    <button
                                        ...
                                        className={`group relative w-16 h-16 rounded-xl flex items-center justify-center transition-all duration-300 border-2
                                            ${isExpanded
                                                ? 'scale-110 shadow-[0_0_20px_rgba(19,200,236,0.2)] border-primary bg-[#111718]'
                                                : 'hover:scale-105 border-[#283639] hover:border-[#3b4f54] bg-[#1d2628]'
                                            }`}
                                        style={{
                                    ...
                                    >
                                        <Icon className={`w-8 h-8 transition-colors ${isExpanded ? 'text-primary' : 'text-[#9db4b9] group-hover:text-white'}`} style={{ color: isExpanded ? color : undefined }} />
                                    ...
                                    <div className="mt-2 flex flex-col items-center">
                                        <span className={`text-[10px] font-bold truncate max-w-[80px] transition-colors ${isExpanded ? 'text-white' : 'text-text-muted'}`}>
                                            {block["@model"].replace('HD2_', '').replace('VIC_', '')}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className={`lg:hidden w-full relative z-10 overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[800px] opacity-100 mt-4 border border-[#283639] bg-[#0b1011] rounded-xl shadow-2xl' : 'max-h-0 opacity-0'}`}>
                                    {isExpanded && (
                                        <BlockParameters
                                            block={block}
                                            blockKey={key}
